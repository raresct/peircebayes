...
class PBCoreTests(unittest.TestCase):
    ...
    def test_coins2(self):
        logd('### COINS2 ###')
        cmd_str = '../peircebayes/peircebayes.py ./coins/coins2_example.pb -d'
    ...
    def test_coin_beta_args(self):
        logd('### COIN BETA ARGS ###')
        cmd_str = '../peircebayes/peircebayes.py ./coins/coin_beta_args.pb  -d'
        PBCoreTests.call_pb(cmd_str)
        thetash = np.load('/tmp/peircebayes/avg_samples.npz')
        thetash = thetash['arr_0']
        thetash = thetash[:, 1]
    ...
    def test_coin_categorical(self):
        ...
        cmd_str = '../peircebayes/peircebayes.py ./coins/coin_categorical.pb  -d'
        PBCoreTests.call_pb(cmd_str)
        thetash = np.load('/tmp/peircebayes/avg_samples.npz')
        thetash = thetash['arr_0']
        thetash = thetash[:, 1]
    ...
    def test_lda1(self):
        logd('### LDA1 ###')
        cmd_str = '../peircebayes/peircebayes.py ./lda/lda_example.pb -d'
        PBCoreTests.call_pb(cmd_str)
        with open('/tmp/peircebayes/aprob/plates/out1.plate', 'r') as fin:
            out1 = fin.read()
    ...
    def test_rim1(self):
        logd('### RIM1 ###')
        cmd_str = '../peircebayes/peircebayes.py ./rim/rim_example1.pb -d'
        PBCoreTests.call_pb(cmd_str)
        with open('/tmp/peircebayes/aprob/plates/out1.plate', 'r') as fin:
            out1 = fin.read()
    ...
    def test_rim2(self):
        logd('### RIM2 ###')
        cmd_str = '../peircebayes/peircebayes.py ./rim/rim_example2.pb -d'
        PBCoreTests.call_pb(cmd_str)
...
class PBOptTests(unittest.TestCase):
    ...
    def test_lda2(self):
        logd('### LDA2 ###')
        cmd_str = ('../peircebayes/peircebayes.py ./lda/lda_example2.pb'+
            ' -n 150 -b 100 -d -t')
        PBCoreTests.call_pb(cmd_str)
        # original params
